...
Page({
   ...
   */
  onLoad(options) {
    console.log('地形图/3D柱状图页面加载，参数:', options);
    
    // 根据URL参数设置视图模式
    const viewMode = options.mode === '3d-bar' ? '3d-bar' : 'terrain';
  ...
  switchViewMode(e) {
    ...
    this.setData({
      ...
      // 如果切换到3D柱状图模式且尚未初始化，则初始化
      if (mode === '3d-bar' && this.data.chartData.length === 0) {
        this.init3DBarChart();
      }
    });
  },
